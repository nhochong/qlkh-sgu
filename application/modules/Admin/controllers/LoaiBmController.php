...
class Admin_LoaiBmController extends Khcn_Controller_Action_Admin
{   
    public function indexAction() 
    {
        // TODO Auto-generated {0}::indexAction() default action
		$this->view->parent_id = $parent_id = $this->_getParam('ma_loai', 0);
        $loaiBMs = Khcn_Api::_()->getDbTable('loai_bieu_mau', 'default')->getByParent($parent_id);
        $paginator = Zend_Paginator::factory($loaiBMs);
        $currentPage = 1;
	...
	public function themAction() 
    {
    	...
    	if($this->getRequest()->isPost())
		{
			...
			{		
				...
				$table = Khcn_Api::_()->getDbTable('loai_bieu_mau', 'default');
				$loai_bieu_mau = $table->createRow();
				$loai_bieu_mau->setFromArray($values);
				$loai_bieu_mau->save();
								
				$_SESSION['msg'] = 'Thành công !. Dữ liệu đã được lưu trữ .';
				$_SESSION['type_msg'] = 'success';
				if($form->submitCon->isChecked()){
					$this->_redirect('/admin/loai-bm/them/ma_loai/' . $ma_loai);					
				}else{
					$this->_redirect('/admin/loai-bm/index/ma_loai/' . $ma_loai);
				}
			}
			else
			{
    ...
    public function xoasAction()
    {
		...
		foreach($_POST['item'] as $id){
			$loai_bm = Khcn_Api::_()->getItem('default_loai_bieu_mau', $id);
			if($loai_bm != NULL)
			{			
				$loai_bm->delete();
			}			
