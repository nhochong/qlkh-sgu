...
class Default_LichThiController extends Khcn_Controller_Action_Standard
{
	/**
    ...
    public function indexAction() 
    {
        ...
        $danhSach = Khcn_Api::_()->getDbTable('lich_thi', 'default')->getPaginator($params);
	...
	public function chiTietAction() 
    {
        ...
        if(!empty($id)){        	
        	$item = Khcn_Api::_()->getItem('default_lich_thi', $id);
        	if($item != null){
        		$item->so_lan_xem += 1;
				$item->save();
	        	$this->view->item = $item;
        	}else{
        		$this->_redirect('lich-thi/index');
        	}
        }else{
        	$this->_redirect('lich-thi/index');
        }
